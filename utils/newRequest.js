...
let inError = false;
const stitchUrlParam = (url, param) => {
  if (!param) return url;
...
const request = async (opt) => {
  ...
  try {
    // const csrfRes = await instance({
    //   url: '/scmOpAdmin/csrfTokenServlet',
    //   method: 'POST',
    //   data:{
    //     csrfType,
    //     t: +new Date(),
    //   }
    // })
    // options.url =  stitchUrlParam(options.url, `csrfType=${csrfType}&csrfToken=${csrfRes.data}`);
    options.url =  stitchUrlParam(options.url);
    const res = await instance(options);
    // console.log(res);
    if ((res.msg !== 'success' && !res.success )&& options.ifHandleError) {
      // 自定义参数，是否允许全局提示错误信息
