...
import styles from './footNavCard.module.less';
class FootNavCard extends React.Component {
  handleGo = path => {
    this.props.onLink({ path });
  };
  render() {
    return (
      <div className={styles['footNavCard-box']}>
        <div className="footNavCard-title">
          <span>海量资源持续帮助您的企业降本增效</span>
        </div>
        <div className="footNavCard-list">
          {(this.props.data || []).map(item => (
            <div className="list-item" key={item.title}>
              <div className="item-num-box">
                <span className="item-num">{item.numText}</span>
                <span className="item-num-unit">{item.unit}</span>
              </div>
        ...
        <div
          className="footNavCard-search"
          onClick={this.handleGo.bind(this, '/hywportal/highwayPortSearch')}
        >
          <svg
            fill="#90C3FF"
            t="1648791533693"
          ...
          <span className="footNavCard-search-text">公路港查询</span>
        </div>
      </div>
    );
  }
